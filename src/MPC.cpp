...
// Set index of state and actuator values
size_t x_start     = 0;
size_t y_start     = N;
...
class FG_eval
{
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars)
  {
    ...
    {
      fg[0] += CppAD::pow(vars[cte_start + t], 2) * 10;
    ...
    for (size_t t = 0; t < N - 1; t++)
    {
      ...
      fg[0] += CppAD::pow((vars[v_start + t] * vars[delta_start + t]), 2) * 10;
    }
    ...
    for (size_t t = 0; t < N - 2; t++)
    {
      ...
      fg[0] += CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2) * 2;
    }
    ...
    for (size_t t = 1; t < N; t++) {
      ...
      // Reference position and desired orientation
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // no limits for non-actuator values
  ...
  // acceleration / braking limited to +/- 1
